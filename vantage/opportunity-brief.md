# VANTAGE — Strategic Opportunity Brief
...
Two prior artefacts sit underneath this brief and are referenced throughout: the **proprietary capability framework** (the scored model) and the **BARS rubric** (`vantage/rubric/bars-rubric.md`), seeded in part by a **structured database of practical communication items** (`vantage/research/`).
...
**The product.** VANTAGE is a coaching *system*, not a course library, a meeting-notes tool, or a narrow speaking assistant. It has two halves joined by a loop:
...
| Keep my data private & compliant | Get insight without sending meeting content or personal data off my work system | GDPR, client/employer confidentiality, recording norms | Local-first processing; only minimal redacted data egresses; BYO-key and on-prem options (§9A.5, §17) |
...
4. **Privacy is architectural: local-first and data-minimising.** Raw recordings are processed on the user's own device or work environment; only the *minimum derived data* needed for the judgement ever leaves it — never raw audio/video by default. Passive capture is opt-in and read-only. This is both a GDPR/enterprise necessity and a trust advantage, designed in from the start rather than bolted on.
...
### 9.4 Reference pipeline (local-first)

The pipeline is split so that **raw media never leaves the user's machine by default**; only a minimal, redacted evidence package crosses the boundary for the frontier judgement.

```
ON-DEVICE  (user's machine / work environment — raw audio & video stay here)
  Ingest (upload or passive capture)
  → ASR + diarisation + word-level timestamps            (local)
  → speaker identification / voiceprint isolates the user (local)
  → feature extraction: prosodic + conversational dynamics + lexical/semantic signals (local)
  → PII / sensitive-content redaction → build the MINIMAL EVIDENCE PACKAGE

MINIMAL EGRESS  (only what the judgement needs, with consent — never raw media)
  redacted evidence spans (timestamp + quote)  +  numeric signals
  [+ optional, consented: user-cropped, ~1 fps frames for the visual-presence layer only]
  → rubric scoring: deterministic metrics + Claude as BARS judge (user-only, evidence-cited)
  → feedback generation (brand voice) → links to modules/principles/drills

LOCAL STORE  (encrypted, user-controlled retention) → dashboard, chat, reports, trend history
```

For environments that forbid *any* egress, an **on-prem / local-model deployment** runs the judge locally too, so nothing leaves the perimeter (§17).
...
- **Access via a VANTAGE key/licence** (issued on purchase) in the prototype; **Stripe** subscription + top-up credits automate issuance at launch.
- **Local-first processing, managed billing — they coexist.** "Managed" governs *who pays and how spend is capped*, not *where raw media goes*. The on-device agent (§9A.5) does the heavy lifting locally and calls the managed proxy with a **minimised payload** (redacted spans + signals, and only consented user-cropped frames for visual presence). The proxy mints provider tokens and meters quota exactly as before; it simply never receives the raw recording.
- **The cost levers are also the privacy levers.** Cropping to the user, ~1 fps sampling, medium resolution, clip selection and de-duping were chosen to bound cost — and they double as **data-minimisation**: the visual layer egresses only a few low-frame-rate frames of the user, not the meeting. None of this lowers analysis quality.
...
VANTAGE is positioned as a **coaching system**, distinct from every adjacent category:
...
4. **Breadth + coaching depth no incumbent spans.** Education has frameworks but no real-time measurement; meeting tools have capture but no development model; speaking tools have feedback but narrow scope; sales tools have depth but only commercially. VANTAGE is the only design that connects framework-rich development to day-to-day behavioural coaching across all five domains.
...
- **Local-first processing & data minimisation (the foundation).** Raw recordings are processed on the user's own device/work environment; transcription, feature extraction and PII/sensitive-content redaction happen locally, and **only a minimal, redacted evidence package leaves the perimeter** for the frontier judgement (never raw audio/video by default; visual presence egresses only consented, user-cropped, low-frame-rate frames). This is GDPR-aligned by construction — lawful basis, purpose limitation, and data minimisation are built into the pipeline, not promised in a policy (§9.4, §9A.5).
- **A deployment ladder for trust boundaries.** Local-first + managed judge (default, minimal egress) → BYO-key (the org's provider boundary) → fully on-prem / local model (**zero egress**) for regulated environments — without lowering analysis quality.
...
- **Privacy perception & recording norms** — mitigated by **local-first processing and data minimisation** (raw media stays on-device; only a minimal redacted package egresses), self-only evaluation, opt-in capture, the on-prem option, and data control (§9.4, §9A.5, §17), but remains an adoption and trust risk to manage.
...
```mermaid
flowchart TD
    subgraph SRC[Inputs to the framework]
      A["Best-in-class training, benchmarked<br/>Exec presence · Leadership · Communication<br/>Negotiation · Sales · Interpersonal"]
      B["Academic assessment science<br/>Bates ExPI · Spitzberg-Cupach · BARS · Kirkpatrick"]
      C["Proprietary practical-item database<br/>Say What You Mean / NVC spine"]
    end
    A --> M
    B --> M
    C --> M
    M["CAPABILITY MODEL + BARS RUBRIC<br/>10 modules → sub-areas → behaviours → drills<br/>(verbal + non-verbal)"]

    G["Customer goal<br/>more credible, influential, senior, calm, clear, effective"] --> M
    G --> CFG
    AI["AI-guided setup<br/>role · goals · pain points → recommended weights"] --> CFG
    CFG["Configuration<br/>default all · manual select · AI-guided weighting"] --> ENG

    M --> LEARN["LEARN — the curriculum<br/>theory · principles · behaviours · examples · drills"]
    M --> ENG

    EV["Evidence capture<br/>meetings · uploads · role-plays · reflections<br/>+ optional passive agent"] --> ENG
    ENG["EVALUATE — the engine ★ the invention ★<br/>holistic, self-only scoring:<br/>overall · module · sub-area · behavioural · priorities"]

    ENG --> COACH["Integrated AI coach<br/>setup · prep · interpret · practise · pattern-coach · navigate"]
    ENG --> OUT["Outputs<br/>post-meeting · daily · weekly · development reports<br/>(dashboard + quoted moments, top 1–3 priorities)"]
    COACH --> OUT
    ENG -->|"routes to the right modules & drills"| LEARN
    LEARN -->|"practise in real conversations"| EV
    OUT --> LOOP["Progress loop<br/>trend tracking · reassessment · targeted improvement<br/>(the transfer problem, solved)"]
    LOOP -.->|"repeat over real life"| EV
```
