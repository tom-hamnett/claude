# VANTAGE — Perplexity master prompt (end-to-end book build)

*This file contains (1) a short SETUP section for you, Tom — do this once before running; and (2) THE MASTER PROMPT — paste everything between the `=== BEGIN ===` and `=== END ===` markers into Perplexity and let it run. The prompt is self-contained: it takes Perplexity from your raw knowledge base, back to first-principles needs, through title and guiding principles, a stress-tested structure, chapter-by-chapter drafting as Perplexity Pages, final assembly into a book, and a built-in end-to-end quality check — all within a 250–300 page target (≤400) and without asking you for anything.*

---

## SETUP (do this once)

1. **Create a Perplexity Space** (Spaces let you attach files and pin instructions that persist across the whole thread). Name it e.g. *"VANTAGE — The Book."*
2. **Upload your knowledge base into the Space.** At minimum:
   ...
   - the two worked curriculum modules in `vantage/curriculum/` (the in-product "house style");
...
You will design, stress-test, draft, assemble, and finally check an entire book, working
autonomously through the eight stages below (Stage 0 through Stage 7). You do not ask the user
clarifying questions. Where information is missing you make the most reasonable assumption,
record it in an ASSUMPTIONS LOG, and proceed. You advance through every stage to completion.
...
Aim for a finished book of ~250–300 pages; the hard upper limit is ~400 pages. Use ~300 words
per page as the planning ratio, giving: TARGET ≈ 75,000–90,000 words; HARD CAP ≈ 120,000 words.
- In Stage 2, set an explicit WORD BUDGET per chapter and for the front/back matter that sums
  within the target (typically 10–14 chapters at ~5,000–7,000 words each).
- In Stage 5, keep each chapter within its budget and maintain a RUNNING WORD COUNT in the Book
  Bible. If the running total trends over target, TIGHTEN (cut redundancy, shorten examples) —
  do not drop any required blueprint section. Never exceed the hard cap.
- Quality and the required structure take priority over hitting an exact number, but the book
  must land within 250–400 pages, ideally 250–300. Report the estimated page count in Stage 7.
...
5. WORD/PAGE BUDGET: assign each chapter and the front/back matter a word budget summing to the
   target (~75,000–90,000 words ≈ 250–300 pages; hard cap ~120,000 ≈ 400). Record budgets in the
   Book Bible. The chapter count and per-chapter budget should reflect each chapter's weight.
...
updated (status: drafted/passed) and maintain the RUNNING WORD COUNT against the budget. Keep
each chapter within its word budget; if you trend over target, tighten rather than cut required
sections.
...
================================================================================
STAGE 7 — FINAL CHECK / QUALITY ASSURANCE (built-in checking at the end)
================================================================================
OBJECTIVE: Verify the finished book end-to-end before sign-off, and fix what you find. Do not
declare the book done until this passes.
TASKS — produce a QA REPORT with an explicit PASS/FAIL and the fix applied for each item:
1. PROMISE AUDIT — every item on the Promise List from the Constitution is explicitly
   delivered; name the chapter/section that delivers each.
2. CITATION & FACT CHECK — every quote is attributed and NOT reproduced verbatim beyond brief
   fair-use; every major claim is sourced at the right tier; no fabricated or mis-attributed
   sources; database anchors (minto_path/source_page) are valid; re-verify any web-sourced facts.
3. NEEDS-HOOK CHECK — the introduction saturates the customer needs, and every chapter and major
   section opens NEED → BENEFIT → content. Flag any section that teaches before motivating.
4. LEARNING-SCIENCE CHECK — retrieval questions, a spaced callback, at least one dual-coding
   diagram, worked-example→faded-practice, and real-world (not role-play) drills are present in
   every chapter.
5. ANTI-BIAS & SELF-ONLY CHECK — no coded-standard bias; effectiveness is judged toward the
   reader's own goals/style; the self-only ("coach you, not the room") frame is intact; the book
   is honest about the limits of its evidence.
6. CONSISTENCY CHECK — title, terminology, model names, voice, formatting, numbering, boxes, and
   cross-references are aligned across the whole book; no forbidden phrases; every chapter
   contains all blueprint sections; every spaced callback points to a real, correct place.
7. ARC & REDUNDANCY CHECK — the conclusion fulfils the opening; the convergence through-line
   recurs; transitions land; no chapter is redundant or missing.
8. LENGTH CHECK — estimate total word count and PAGE COUNT (words ÷ 300). Confirm it lands in
   the 250–300 target and under the 400 cap. If over, tighten; if well under target, identify
   where depth should be added, and add it.
9. READABILITY & USABILITY CHECK — a reader can enter any chapter cold and act on it; boxes,
   tables and diagrams are correctly placed; the text is scannable.
THEN: apply all fixes (log them in the CHANGE LOG) and issue a FINAL READINESS VERDICT —
"READY" with every item passed, or "NOT READY" with a precise punch-list, each item assigned a
fix. If NOT READY, perform the fixes and re-run this stage until READY.
OUTPUT: the QA REPORT (item-by-item), the estimated PAGE COUNT, the CHANGE LOG of fixes, and the
FINAL READINESS VERDICT. Re-print any chapter/section you materially changed.
SELF-CHECK GATE: Verdict is READY; all nine checks pass; page count is within range.
...
BEGIN NOW with Stage 0. Work autonomously through all eight stages (Stage 0 through Stage 7),
ending only when Stage 7 returns a READY verdict. Do not ask the user questions; record
assumptions and proceed. When a turn is cut off by length, resume exactly where you stopped on
"continue".
