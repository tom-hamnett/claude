...
export default function PendingIngestions({ programmeId, onIngestFile, onRefresh }) {
  ...
  async function ingestDocument(id) {
    setIngesting(id);
    try {
      const r = await fetch(`/api/pending-ingestions/${id}/ingest-document`, { method: "POST" });
      const result = await r.json();
      if (!r.ok) throw new Error(result.error);
      load();
      if (onRefresh) onRefresh();
    } catch (e) {
      alert(`Document extraction failed: ${e.message}`);
    }
    setIngesting(null);
  }
  ...
  const structured = pending.filter(p => isStructuredFile(p.filename));
  const documents = pending.filter(p => !isStructuredFile(p.filename));
  ...
  return (
    <div style={{ background: "rgba(245,197,68,0.08)", border: "1px solid rgba(245,197,68,0.3)", borderRadius: 10, padding: "14px 18px", marginBottom: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
        <div>
          ...
          <div style={{ fontSize: 12, fontFamily: "var(--font-b)", color: "var(--text2)" }}>
            New or changed files detected by the sync scheduler.
            {structured.length > 0 && documents.length > 0 && ` ${structured.length} spreadsheet(s) for data tables, ${documents.length} document(s) for AI library.`}
          </div>
        </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {pending.map(p => {
          const isData = isStructuredFile(p.filename);
          const isExtracting = ingesting === p.id;
          return (
            <div key={p.id} style={{ background: "var(--bg3)", border: "1px solid var(--border)", borderLeft: `3px solid ${isData ? "#F5C544" : "#9B7FE6"}`, borderRadius: 6, padding: "10px 14px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
                  <div style={{ fontSize: 13, fontFamily: "var(--font-d)", fontWeight: 600, color: "var(--text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.filename}</div>
                  <span style={{ fontSize: 9, fontFamily: "var(--font-m)", color: isData ? "#F5C544" : "#9B7FE6", background: isData ? "rgba(245,197,68,0.15)" : "rgba(155,127,230,0.15)", padding: "1px 6px", borderRadius: 3, flexShrink: 0 }}>
                    {isData ? "DATA" : "DOC"}
                  </span>
                </div>
              ...
              <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                {isData ? (
                  <button onClick={() => onIngestFile(p)} style={{ fontSize: 11, fontFamily: "var(--font-d)", fontWeight: 700, padding: "6px 14px", borderRadius: 4, background: "var(--accent)", color: "#000" }}>Ingest Data →</button>
                ) : (
                  <button onClick={() => ingestDocument(p.id)} disabled={isExtracting} style={{ fontSize: 11, fontFamily: "var(--font-d)", fontWeight: 700, padding: "6px 14px", borderRadius: 4, background: "#9B7FE6", color: "#fff" }}>
                    {isExtracting ? <Spinner s={10} /> : "Extract Text →"}
                  </button>
                )}
