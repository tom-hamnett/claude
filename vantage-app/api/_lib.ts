// Shared helpers for the VANTAGE managed proxy (Vercel serverless functions).
// Holds provider keys server-side, validates a VANTAGE licence key, and meters
...
const PLAN = {
  videoSec: Number(process.env.VANTAGE_PLAN_VIDEO_MIN || 60) * 60,
  audioSec: Number(process.env.VANTAGE_PLAN_AUDIO_MIN || 600) * 60,
};

export function cors(req: VercelRequest, res: VercelResponse): boolean {
  res.setHeader('Access-Control-Allow-Origin', process.env.VANTAGE_ALLOW_ORIGIN || '*');
  res.setHeader('Access-Control-Allow-Headers', 'content-type, x-vantage-key');
...
export function licenceFrom(req: VercelRequest): string {
  return (req.headers['x-vantage-key'] as string) || (req.body && (req.body as any).license) || '';
}
...
export async function reserve(license: string, kind: 'video' | 'audio' | 'transcript', sec: number) {
  if (!license && process.env.VANTAGE_DEV_OPEN !== 'true') return { ok: false, status: 401, reason: 'Missing VANTAGE key.' };
  const store = await kv();
