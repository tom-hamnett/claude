...
export const openaiProvider: AIProvider = {
  ...
  async complete(req: AICompleteRequest, opts) {
    ...
    try {
      ...
      throw new AIError(err instanceof Error ? err.message : 'Network error contacting OpenAI.', {
        provider: 'openai',
      });
    }
    ...
    if (!resp.ok) {
      ...
      throw new AIError(`OpenAI ${resp.status}: ${friendlyMessage(raw) || resp.statusText}`, {
        provider: 'openai',
        status: resp.status,
        raw,
      });
