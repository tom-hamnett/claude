...
- **AI-native, BYO-key, serverless.** Your Anthropic API key lives only in your
  ...
  There is no VANTAGE server. Without a key the app runs a transparent offline
...
`api/` and the env vars in `.env.example`. Users only need a VANTAGE key.

## Privacy

Everything stays on your device. VANTAGE evaluates and reports on **you only** —
