// Client helpers for the managed (VANTAGE-hosted) proxy. Used when the user is
// on a VANTAGE key instead of their own provider keys. Large media is uploaded
...
const headers = (key: string) => ({ 'content-type': 'application/json', 'x-vantage-key': key });
...
export async function managedUsage(key: string): Promise<any | null> {
  try {
    const r = await fetch('/api/usage', { headers: { 'x-vantage-key': key } });
