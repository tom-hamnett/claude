...
export default function Coach() {
  ...
  useEffect(() => {
    getSettings().then((s) => setHasKey(!!s.apiKey?.trim() || s.aiMode === 'local' || (s.aiMode === 'managed' && !!s.vantageKey?.trim())));
  }, []);
  useEffect(() => {
  ...
  async function send(text: string) {
    ...
    const isManaged = settings.aiMode === 'local' || (settings.aiMode === 'managed' && !!settings.vantageKey?.trim());
    if (!settings.apiKey?.trim() && !isManaged) {
      setError('The coach needs an Anthropic key (BYO), a VANTAGE key (Managed), or the local server (Self-hosted). Set it in Settings.');
      return;
    }
